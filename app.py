...
def send_api_key_email(to_email, api_key):
    if not RESEND_API_KEY:
        ...
        return
    payload = {"from": f"UA Parser API <{RESEND_FROM_EMAIL}>", "to": [to_email], "subject": "Your UA Parser API Key is Ready", "html": f"<h2>Thanks for your purchase!</h2><p>Your API key is ready to use:</p><pre style=\"background:#f4f4f4;padding:10px;border-radius:5px;\">{escape(api_key)}</pre><p>Use it like this: <code>curl -H \"X-API-Key: {escape(api_key)}\" {BASE_URL}/v1/parse?ua=...</code></p><p>You have 1000 credits. Each request uses 1 credit.</p>"}
