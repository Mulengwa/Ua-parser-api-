...
USDT_PRICE_USD = float(os.environ.get("USDT_PRICE_USD", "5.00")) # Price of 1000 credits, default $5
...
def init_db():
    ...
    with psycopg.connect(DATABASE_URL, sslmode='require') as conn: # Connect to Postgres with SSL
        with conn.cursor() as cur: # Create cursor for SQL queries
            cur.execute(""" # Create api_keys table if not exists
                CREATE TABLE IF NOT EXISTS api_keys (
                    key TEXT PRIMARY KEY, # API key is primary key
                    credits INT NOT NULL DEFAULT 0, # Credit balance
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP # Last update time
                )
            """)
            cur.execute(""" # Create orders table if not exists
                CREATE TABLE IF NOT EXISTS orders (
                    order_id TEXT PRIMARY KEY, # Order ID from NowPayments
                    api_key TEXT, # API key to credit after payment
                    email TEXT, # Customer email
                    provider TEXT DEFAULT 'nowpayments', # Payment provider
...
def create_order():
    ...
    np_payload = { # Payload for NowPayments invoice API
        "price_amount": USDT_PRICE_USD, # $5.00
        "price_currency": "usd", # Price in USD
        "pay_currency": "usdttrc20", # Accept USDT on TRON network
        "order_id": order_id, # Our internal order ID
        "order_description": "UA Parser API - 1000 credits", # Description shown to customer
        "ipn_callback_url": "https://ua-parser-api-zsql.onrender.com/webhook/nowpayments", # Webhook URL
        "success_url": "https://ua-parser-api-zsql.onrender.com/thanks", # Redirect after payment
        "cancel_url": "https://ua-parser-api-zsql.onrender.com/" # Redirect if cancelled
    }
