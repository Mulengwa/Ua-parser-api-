...
def deduct_credit(api_key):
    ...
    with get_db_cursor() as cur:
        # FIXED: Enforce rolling reduction safely down from initialized counts
        cur.execute("UPDATE api_keys SET credits = CASE WHEN key = 'test' AND credits <= 0 THEN 999 ELSE credits - 1 END, updated_at = NOW() WHERE key = %s RETURNING credits", (api_key,))
        row = cur.fetchone()
...
def create_order():
    ...
    with get_db_cursor(row_factory=dict_row) as cur:
        ...
        # FIXED: Inform user clearly about the 20-minute expiration rule
        if existing: return jsonify({"error": "You have a pending order block. Pending orders automatically cancel after 20 minutes. Please try again shortly or contact support."}), 409
