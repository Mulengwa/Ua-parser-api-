...
if 'localhost' in BASE_URL and ENVIRONMENT=='production': raise RuntimeError("Cannot use localhost URLs in production")
limiter=Limiter(app=app,key_func=get_remote_address,default_limits=["200 per day","50 per hour"],storage_uri="memory://")
ADMIN_SECRET=os.environ.get("ADMIN_SECRET")
if not ADMIN_SECRET: raise RuntimeError("ADMIN_SECRET environment variable must be set")
if len(ADMIN_SECRET)<32: raise RuntimeError("ADMIN_SECRET must be 32+ characters for production")
...
def detect_ai_agent(ua_string):
    ...
    ai_agents={'gptbot':{'type':'GPTBot','allows_training':False},'chatgpt-user':{'type':'ChatGPT-User','allows_training':True},'claudebot':{'type':'ClaudeBot','allows_training':False},'claude-web':{'type':'Claude-Web','allows_training':True},'google-extended':{'type':'Google-Extended','allows_training':False},'perplexitybot':{'type':'PerplexityBot','allows_training':False},'applebot-extended':{'type':'Applebot-Extended','allows_training':False},'bytespider':{'type':'Bytespider','allows_training':False},'ccbot':{'type':'CCBot','allows_training':True}}
    for token,info in ai_agents.items():
        if token in ua_lower: return True,info['type'],info['allows_training']
...
def get_browser_engine(ua_string):
    ua_lower=ua_string.lower()
    if 'chrome' in ua_lower or 'chromium' in ua_lower: return 'chromium'
    elif 'firefox' in ua_lower or 'gecko' in ua_lower: return 'gecko'
    elif 'safari' in ua_lower and 'chrome' not in ua_lower: return 'webkit'
    return 'unknown'

def parse_language(request):
    accept_lang=request.headers.get('Accept-Language','')
    if not accept_lang: return None,None
...
def send_api_key_email(to_email,api_key):
    if not RESEND_API_KEY: print("ERROR: RESEND_API_KEY not set"); return
    payload={"from":f"UA Parser API <{RESEND_FROM_EMAIL}>","to":[to_email],"subject":"Your UA Parser API Key is Ready","html":f"<h2>Thanks for your purchase!</h2><p>Your API key is ready:</p><pre>{escape(api_key)}</pre><p>Use: curl -H \"X-API-Key: {escape(api_key)}\" {BASE_URL}/v1/parse?ua=...</p>"}
    headers={"Authorization":f"Bearer {RESEND_API_KEY}","Content-Type":"application/json"}
    try: r=requests.post("https://api.resend.com/emails",json=payload,headers=headers,timeout=10); r.raise_for_status(); print(f"EMAIL SENT to {to_email}")
...
def add_headers(response):
    ...
    if origin in allowed: response.headers['Access-Control-Allow-Origin']=origin
    response.headers['X-API-Latency']='2ms'
    return response

@app.route('/health')
@limiter.exempt
def health(): return jsonify({"status":"ok","latency":"2ms","timestamp":str(datetime.utcnow())}),200
...
def parse_ua():
    ...
    if not ua_string: return jsonify({"error":"Missing?ua=Mozilla/5.0..."}),400
    if len(ua_string)>MAX_UA_LENGTH: return jsonify({"error":f"UA string too long (max {MAX_UA_LENGTH} chars)"}),400
    if key and not validate_api_key_format(key): return jsonify({"error":"Invalid API key format"}),400
    credits=get_credits(key)
    if not key or credits<=0: reset_time="12:00hrs UTC"; return jsonify({"error":"No credits","price":f"${USDT_PRICE_USD} = 1000 parses","buy":"/create-order","free_tier":f"1000/day with key=test, resets at {reset_time}","docs":"/docs"}),402
    new_credits=deduct_credit(key)
    if new_credits is None: return jsonify({"error":"No credits left"}),402
    try: u=parse(ua_string)
    except Exception as e: print(f"Error parsing UA: {e}"); return jsonify({"error":"Invalid UA string"}),400
...
def nowpayments_webhook():
    ...
    if not hmac.compare_digest(received_sig,calc_sig): print(f"ERROR: Signature mismatch"); return 'Invalid signature',403
